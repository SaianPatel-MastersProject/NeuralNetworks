...
net = importNetworkFromONNX("SteeringModel_Iteration21.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    input_i = [CTE,curvature, dCurvature, HeadingError, lookAhead_i];

    steeringOutput_i = predict(net, input_i);

    % Populate the array
    NN_Sim_Data(i,2:10) = input_i;
    NN_Sim_Data(i,11) = steeringOutput_i;
...
hold on
plot(NN_Sim_Data(:,1), NN_Sim_Data(:,11))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;

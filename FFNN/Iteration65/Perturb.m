...
net = importNetworkFromONNX("SteeringModel_Iteration65.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    input_i = [CTE, HeadingError, lookAhead_i, dHE(i)];
...
for i = 1:nInputs
    ...
    MAE_i = abs(steerAngle - NN_Sim_Data(:, end));
    MAE(1, i) = sum(MAE_i) / nEntries;

    MAE_i = abs(steerAngle - sPlus(:, i));
    MAE(2, i) = sum(MAE_i) / nEntries;

    MAE_i = abs(steerAngle - sNeg(:, i));

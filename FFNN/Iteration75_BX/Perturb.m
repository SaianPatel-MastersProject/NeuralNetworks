...
net = importNetworkFromONNX("SteeringModel_Iteration75_BX.onnx", "InputDataFormats", "BC");
...
matFilePath = 'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP03_31\2025_FYP03_31_D5_R08.mat';
lap = 21;
...
%% Perturb
nInputs = size(NN_Sim_Data, 2) - 2;

sPlus = zeros([size(NN_Sim_Data, 1), 1]);
sNeg = sPlus;

% Perturb each of the inputs
for i = 1:nInputs

    fprintf("Perturbing input %i \n", i);

    iPlus = NN_Sim_Data(:, i + 1) .* (1 + 10/100);
    iNeg = NN_Sim_Data(:, i + 1) .* (1 - 10/100);
    ...
    for j = 1:size(iPlus, 1)
        ...
        sPlus(j,i) = predict(net, inputPlus);
        sNeg(j,i) = predict(net, inputNeg);
...
%% Calculate MAE of perturbed inputs

MAE = zeros([3, nInputs]);
steerAngle = runStruct.data.steerAngle;
nEntries = size(steerAngle, 1);

for i = 1:nInputs

    MAE_i = (steerAngle - NN_Sim_Data(:, end)).^2;
    MAE(1, i) = sum(MAE_i) / nEntries;

    MAE_i = (steerAngle - sPlus(:, i)).^2;
    MAE(2, i) = sum(MAE_i) / nEntries;

    MAE_i = (steerAngle - sNeg(:, i)).^2;
    MAE(3, i) = sum(MAE_i) / nEntries;

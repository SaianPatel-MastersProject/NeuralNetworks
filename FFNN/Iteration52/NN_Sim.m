...
net = importNetworkFromONNX("SteeringModel_Iteration52.onnx", "InputDataFormats", "BC");

%%
trainingData_Sim = readtable('TrainingDataNorm.csv', 'VariableNamingRule','preserve');

minMaxArray = zeros([2, 4]);
...
[dLookOverall, kappaSorted] = Utilities.fnLookAheadDistanceSigmoidCurvature(6, 60, 20000, 2e-4, kappaInterp);
...
for i = 1:size(runStruct.data, 1)
    ...
    dLookAhead = interp1(kappaSorted, dLookOverall, (kappaInterp(closestWaypointIdx))^2 );

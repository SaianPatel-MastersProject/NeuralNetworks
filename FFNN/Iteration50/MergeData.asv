...
for i = 1:size(data, 2)
    ...
    for j = 1:size(data_i, 1)
        ...
        % Calculate new dCurvature
        dCurvatureCol(j,1) = (lookAhead_i - lookAhead_0) / (dLookAhead - dLookAhead_0);


    end

    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, dCurvatureCol, data_i.steerAngle ];

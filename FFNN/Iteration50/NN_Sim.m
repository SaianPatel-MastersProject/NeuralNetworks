...
net = importNetworkFromONNX("SteeringModel_Iteration50.onnx", "InputDataFormats", "BC");

%%
trainingData_Sim = readtable('TrainingDataNorm.csv', 'VariableNamingRule','preserve');

minMaxArray = zeros([2, 5]);
...
for i = 1:size(runStruct.data, 1)
    ...
    dCurvature_i = (lookAhead_i - lookAhead_0) / (dLookAhead - 3);

    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i, dCurvature_i];

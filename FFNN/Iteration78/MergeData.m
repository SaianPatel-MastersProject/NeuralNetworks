...
for i = 1:size(data, 2)
    ...
    % Calculate yaw rate - like in the Simulink model
    delta = data_i.steerAngle;
    delta(abs(delta) < 0.01) = 0;
    yawRate = data_i.steerAngle * -1.5;

    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, yawRate ];
...
stanTable = array2table(stanArray, 'VariableNames', columnNames(1:end));

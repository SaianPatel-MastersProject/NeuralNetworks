...
net = importNetworkFromONNX("SteeringModel_Iteration80.onnx", "InputDataFormats", "BC");
...
for i = (spacing*(sequence_length-1) + 1):size(NN_Sim_Data,1)

    input_i = [NN_Sim_Data(i-(3*spacing), 2:5), NN_Sim_Data(i-(2*spacing), 2:5), NN_Sim_Data(i-(1*spacing), 2:5), NN_Sim_Data(i, 2:5)];
    NN_Sim_Data(i, 6) = predict(net, input_i);

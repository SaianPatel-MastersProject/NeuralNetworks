...
net = importNetworkFromONNX("SteeringModel_Iteration102.onnx", "InputDataFormats", "BC");
...
for i = 21:size(runStruct.data, 1)
    ...
    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i, lookAheadHE_i, projectedCTE];
    ...
    % Populate the array
    NN_Sim_Data(i,2:7) = input_i;
    NN_Sim_Data(i,8) = steeringOutput_i;
...
hold on
plot(NN_Sim_Data(:,1), NN_Sim_Data(:,8))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;

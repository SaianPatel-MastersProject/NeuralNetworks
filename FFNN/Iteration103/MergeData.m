...
runsLaps = {
    ...
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP04_14\2025_FYP04_14_D7_R02.mat', [1], 'BAR';...% BAR
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP04_14\2025_FYP04_14_D7_R03.mat', [1], 'BAR';...% BAR
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP04_14\2025_FYP04_14_D7_R04.mat', [1], 'BAR';...% BAR
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP04_14\2025_FYP04_14_D7_R05.mat', [1], 'BAR';...% BAR
};
...
for i = 1:size(data, 2)
    ...
    %% Projected pos
    yawRate = data_i.steerAngle  .* -1.5;
    yawAngle = zeros([numel(yawRate), 1]);
    for j = 2:numel(yawAngle)

        % yawAngle(i,1) = yawAngle(i-1) + yawRate(i-1) * 0.01;
        yawAngle(j,1) = psi(j-1) + yawRate(j-1) * 0.01;


    end
    yawAngle(1,1) = yawAngle(2,1);

    dSteerAngle = [0; diff(data_i.steerAngle)] ./ 0.01; % Dont convert to degrees

    previewTime = 0.2;
    v = 40;
    
    xF = zeros([numel(yawRate), 1]);
    yF = xF;
    yawFuture = xF;

    for j = 21:numel(yawAngle)

        dSteer_i = dSteerAngle(j-20);

        yawAngle_i = yawAngle(j-20);

        steerAngle_i = data_i.steerAngle(j-20);

        xV = data_i.posX(j);

        yV = data_i.posY(j);

        [xF_i, yF_i, psiF] = Utilities.fnPredictFuturePosition(xV, yV, steerAngle_i, dSteer_i, yawAngle_i, previewTime, v);
    ...
    xF(1:20,1) = data_i.posX(1:20);
    yF(1:20,1) = data_i.posY(1:20);
    yawFuture(1:20,1) = yawFuture(21,1);

    %% Assign
    for j = 20+1:size(data_i, 1)
        ...
        [projectedCTE_j, ~, lookAheadHE_j] = PostProcessing.PE.fnCalculatePathError([xF(j), yF(j), yawFuture(j)], AIW_Data);
        lookAheadHE(j,1) = lookAheadHE_j;

        % Project Errors
        % [projectedCTE_j, ~, ~, ~, ~] = PostProcessing.PE.fnCalculateProjectedPathError([xV, yV, thetaV], AIW_Data, dLookAhead);
    ...
    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, lookAheadHE, projectedCTE, prevSteering1, data_i.steerAngle ];
...
trainingDataNorm = minMaxNormalize(trainingData(:, 1:7));
trainingDataNorm = [trainingDataNorm, trainingData(:,8)];
...
%% View Input Space
plotInputSpace(trainingDataStand, [1:8]);

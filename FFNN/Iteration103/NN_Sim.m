...
net = importNetworkFromONNX("SteeringModel_Iteration103.onnx", "InputDataFormats", "BC");
...
dSteerAngle = [0; diff(runStruct.data.steerAngle)] ./ 0.01; % Dont convert to degrees
...
for j = 21:numel(yawAngle)
    ...
    steerAngle_i = runStruct.data.steerAngle(j-20);

    xV = runStruct.data.posX(j);

    yV = runStruct.data.posY(j);
...
xF(1,1) = runStruct.data.posX(1);
yF(1,1) = runStruct.data.posY(1);
yawFuture(1,1) = yawFuture(2,1);


for i = 21:size(runStruct.data, 1)
    ...
    % [projectedCTE, ~, ~, ~, ~] = PostProcessing.PE.fnCalculateProjectedPathError([xV, yV, thetaV], AIW_Data, dLookAhead);
    [projectedCTE, ~, lookAheadHE_i] = PostProcessing.PE.fnCalculatePathError([xF(i), yF(i), yawFuture(i)], AIW_Data);
    ...
    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i, lookAheadHE_i, projectedCTE, prevSteering1];
    ...
    % Populate the array
    NN_Sim_Data(i,2:8) = input_i;
    NN_Sim_Data(i,9) = steeringOutput_i;
...
hold on
plot(NN_Sim_Data(:,1), NN_Sim_Data(:,9))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;

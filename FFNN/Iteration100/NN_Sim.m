...
net = importNetworkFromONNX("SteeringModel_Iteration100.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    FF_input_i = [input_i(1), input_i(3), input_i(2)];
    FB_input_i = [input_i(4), input_i(5), input_i(6)];

    % combined_input = [FF_input_i, FB_input_i];
    ...
    % Save the gate activations
    % NN_Sim_Data(i, 9) = predict(net, FF_input_i, FB_input_i, 'Outputs', 'StatefulPartition_59');
...
%% Comparing NN and SP MSteer
dSteerH = [0; diff((runStruct.data.steerAngle).*225)]./0.01;
MSteerH = 1/(numel(NN_Sim_Data(:,1)) * 0.01) * trapz(NN_Sim_Data(:,1), abs(dSteerH));

dSteerNN = [0; diff(movmean(NN_Sim_Data(:,8),13).*225)]./0.01;
MSteerNN = 1/(numel(NN_Sim_Data(:,1)) * 0.01) * trapz(NN_Sim_Data(:,1), abs(dSteerNN));

figure;
subplot(2,1,1)
plot(NN_Sim_Data(:,1), runStruct.data.steerAngle)
hold on
plot(NN_Sim_Data(:,1), movmean(NN_Sim_Data(:,8),13))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;
...
subplot(2,1,2)
plot(NN_Sim_Data(:,1), dSteerH)
hold on
plot(NN_Sim_Data(:,1), dSteerNN)
xlabel('Time (s)')
ylabel('dSteerAngle')
grid;
grid minor;
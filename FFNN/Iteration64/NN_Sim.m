...
net = importNetworkFromONNX("SteeringModel_Iteration64.onnx", "InputDataFormats", "BC");
...
% Downsample
timeData = downsample(runStruct.data.time, 10, 2);
headingErrorData = downsample(runStruct.data.HeadingError, 10, 2);
% Re-interpolate
headingErrorDataInterp = interp1(timeData, headingErrorData, runStruct.data.time, 'spline');
% Overwrite the heading error data
runStruct.data.HeadingError = headingErrorDataInterp;
...
dHE =[0; diff(runStruct.data.HeadingError)] ./ (0.1);

...
for i = 1:size(runsLaps, 1)
    ...
    matFilePath = runsLaps{i,1};
     ...
     % Downsample - heading error only
     timeData = downsample(runData.time, 10, 2);
     headingErrorData = downsample(runData.HeadingError, 10, 2);
     % Re-interpolate
     headingErrorDataInterp = interp1(timeData, headingErrorData, runData.time, 'spline');
     % Overwrite the heading error data
     runData.HeadingError = headingErrorDataInterp;
...
for i = 1:size(data, 2)
    ...
    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, dHE, data_i.steerAngle ];
...
%% View Input Space
plotInputSpace(trainingDataNorm, [1:5]);

...
for i = 1:size(data, 2)
    ...
    prevSteer = zeros([size(data_i, 1), 5]);

    for j = 1:size(data_i, 1)

        prevSteer_k = [];
        
        if j <= nPrevSteerPoints * prevSteerSpacing

            % Append to array
            prevSteer(j, :) = [0, 0, 0, 0, 0];

        else

            for k = 1:nPrevSteerPoints

                prevSteer_k(1,k) = data_i.steerAngle(j - (k*prevSteerSpacing));

            end

            % Append to array
            prevSteer(j, :) = prevSteer_k;
    ...
    dataArray_i = [data_i.CTE, ([0; diff(data_i.CTE)]), curvatureCol, ([0; diff(curvatureCol)]), data_i.HeadingError, prevSteer, data_i.steerAngle ];
    
    % Remove first n rows
    dataArray_i = dataArray_i(nPrevSteerPoints * prevSteerSpacing +1:end, :);

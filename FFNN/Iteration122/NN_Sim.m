...
net = importNetworkFromONNX("SteeringModel_Iteration122.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    if i > 40
        ...
        prevSteering2 = runStruct.data.steerAngle(i-40);
    ...
    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i,prevSteering1*-1.5, projectedCTE, lookAheadHE_i];
    ...
    % Populate the array
    NN_Sim_Data(i,2:12) = input_i;
    NN_Sim_Data(i,13) = steeringOutput_i;
...
hold on
plot(NN_Sim_Data(:,1), NN_Sim_Data(:,13))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;

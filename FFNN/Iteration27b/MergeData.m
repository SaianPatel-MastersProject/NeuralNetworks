%% Runs and Laps
runsLaps = {
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2024\FYP12_09\2024_FYP12_09_D4_R17.mat';
};
...
for i = 1:size(runsLaps, 1)
    ...
    matFilePath = runsLaps{i,1};
     ...
     % Only keep L3
     lapData = runStruct.data(runStruct.data.lapNumber == 3, :);

     runData = lapData;

     % Duplicate the data n times
     nDuplication = 100;
     for i = 2:nDuplication

         runData = [runData; lapData];
...
for i = 1:size(data, 2)
    ...
    dataArray_i = [data_i.CTE, curvatureCol, data_i.HeadingError, lookAheadKappa, data_i.steerAngle ];
...
%% Add standardisation of data
nCols = size(dataArray, 2);
for i = 1:nCols

    minVal = min(dataArray(:, i));
    maxVal = max(dataArray(:, i));

    dataArrayStandard(:, i) = 2 * (dataArray(:, i) - minVal)/(maxVal - minVal) - 1;



end

dataArray = dataArrayStandard;
...
%% View Input Space
plotInputSpace(trainingData, [1:8]);
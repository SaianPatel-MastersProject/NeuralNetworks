...
net = importNetworkFromONNX("SteeringModel_Iteration27b.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    CTE = runStruct.data.CTE(i) + ((2 * noiseOrder) * rand() - noiseOrder);
    dCTEcol = [0; diff(runStruct.data.CTE)];
    dCTE = dCTEcol(i) + ((2 * noiseOrder) * rand() - noiseOrder);
    curvature = (1./runStruct.data.rCurvature(i)) + ((2 * noiseOrder) * rand() - noiseOrder);
    curvatureCol = 1./runStruct.data.rCurvature;
    dCuravtureCol = [0; diff(1./runStruct.data.rCurvature)];
    dCurvature = dCuravtureCol(i) + ((2 * noiseOrder) * rand() - noiseOrder);
    HeadingError = runStruct.data.HeadingError(i) + ((2 * noiseOrder) * rand() - noiseOrder);
...
figure;
% subplot(5,1,1)
plot(NN_Sim_Data(:,1), runStruct.data.steerAngle, 'LineWidth', 1)
hold on

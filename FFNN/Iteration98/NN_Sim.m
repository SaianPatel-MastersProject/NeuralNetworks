...
net = importNetworkFromONNX("SteeringModel_Iteration98.onnx", "InputDataFormats", ["BC","BC"]);
...
for i = 1:size(runStruct.data, 1)
    ...
    steeringOutput_i = predict(net, FF_input_i, FB_input_i);
...
%% Plot results with gate activation
figure;
subplot(3,1,1)
plot(NN_Sim_Data(:,1), runStruct.data.steerAngle)
hold on
...
hold on
plot(NN_Sim_Data(:,1), 1-NN_Sim_Data(:,9))
yline(0.5, 'Color', 'black')
xlabel('Time (s)')
ylabel('Gating Weight')
legend({'Current State', 'Future State'})
grid;
grid minor;

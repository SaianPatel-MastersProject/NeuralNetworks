...
for i = 1:size(data, 2)
    ...
    yawRateCol = data_i.steerAngle * -1.5;
    yawRateCol(abs(yawRateCol) < 0.01 ) = 0;
    dYawRateCol = [diff(yawRateCol); 0];
    dataArray_i = [data_i.CTE, ([0; diff(data_i.CTE)]), 1./data_i.rCurvature, ([0; diff(1./data_i.rCurvature)]), dYawRateCol, data_i.steerAngle ];
...
writetable(trainingData, 'TrainingData_b.csv');
...
net = importNetworkFromONNX("SteeringModel_Iteration68.onnx", "InputDataFormats", ["BC", "BC"]);
...
for i = 1:size(runStruct.data, 1)
    ...
    steeringOutput_i = predict(net, input_i([1, 3]), input_i([2, 4]) );
...
for i = 1:nInputs
    ...
    for j = 1:size(iPlus, 1)
        ...
        sPlus(j,i) = predict(net, inputPlus([1,3]), inputPlus([2,4]));
        sNeg(j,i) = predict(net, inputNeg([1,3]), inputNeg([2,4]));

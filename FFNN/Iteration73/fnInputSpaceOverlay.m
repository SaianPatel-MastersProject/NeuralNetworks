...
function fnInputSpaceOverlay(trainingData, runData, trainingIdx)
    ...
    lookAhead1 = zeros([size(runData, 1), 1]);

    % Loop through data to create look-ahead
    for i = 1:size(runData, 1)
        ...
        dLookAhead = interp1(kappaSorted, dLookOverall, abs(runData.kappa(i)));
        iLookAhead = round(dLookAhead/0.1);

        % Look Ahead in Kappa
        lookAhead1(i) = Utilities.fnGetLookAheadValues(kappaInterp, closestWaypointIdx, iLookAhead, 1);


    end
    % Create the equivalents of the training data for the run data (with
    % the same column names)
    runData.lookAhead1 = lookAhead1;
    ...
    for i = 1:nInputs
        ...
        plot(tLap, maxVals, 'Color', 'black', 'LineStyle', '--')
        plot(tLap, minVals, 'Color', 'black', 'LineStyle', '--')

        plot(tLap, runData.(inputColumns{i}))

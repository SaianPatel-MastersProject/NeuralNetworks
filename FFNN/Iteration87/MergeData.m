...
for i = 1:size(data, 2)
    ...
    projectedHE = zeros([size(data_i, 1), 1]);
    dX = diff(data_i.posX);
    dY = diff(data_i.posY);
    psi = atan2(dY, dX);
    psi = [psi; psi(end)];
    ...
    for j = 1:size(data_i, 1)
        ...
        [projectedCTE_j, ~, projectedHE_j, ~, ~] = PostProcessing.PE.fnCalculateProjectedPathError([xV, yV, psi(j)], AIW_Data, 40);

        projectedCTE(j,1) = projectedCTE_j;
        projectedHE(j,1) = projectedHE_j;
    ...
    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, projectedCTE, projectedHE, data_i.steerAngle ];

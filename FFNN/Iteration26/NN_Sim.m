...
net = importNetworkFromONNX("SteeringModel_Iteration26.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    kappaClosest(i,1) = kappa(closestWaypointIdx);
...
for i = 1:size(runStruct.data, 1)
    ...
    dCuravtureCol = [0; diff(kappaClosest)];
    ...
    input_i = [CTE, dCurvature, HeadingError, lookAhead_i];

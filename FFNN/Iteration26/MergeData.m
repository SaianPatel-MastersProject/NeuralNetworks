...
for i = 1:size(data, 2)
    ...
    kappaClosest = zeros([size(data_i, 1), 1]);
    ...
    for j = 1:size(data_i, 1)
        ...
        % Get kappa
        kappaClosest(j) = kappa(closestWaypointIdx);
    ...
    % Trim kappa closest
    % kappaClosest([0; diff(kappaClosest)] > 1e-3) = 0;

    dataArray_i = [data_i.CTE, ([0; diff(curvatureCol)]), data_i.HeadingError, lookAheadKappa, data_i.steerAngle ];
...
%% View Input Space
plotInputSpace(trainingData, [1:2]);
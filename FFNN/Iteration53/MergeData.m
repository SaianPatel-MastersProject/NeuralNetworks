...
for i = 1:size(data, 2)
    ...
    dCurvatureCol = [0; diff(curvatureCol) ./ 0.01];

    lookAheadKappa_0 = zeros([size(data_i, 1), 1]);
    lookAheadKappa = zeros([size(data_i, 1), 4]);
    ...
    for j = 1:size(data_i, 1)
        ...
        % Look Ahead in Kappa
        lookAhead_i = Utilities.fnGetLookAheadValues(kappaInterp, closestWaypointIdx, iLookAhead, 4);
...
plotInputSpace(trainingDataNorm, [1:8]);

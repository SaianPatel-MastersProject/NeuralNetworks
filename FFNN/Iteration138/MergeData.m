...
runsLaps = {
    ...
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP05_19\2025_FYP05_19_D2_R02.mat', [2:9], 'SUZ', [-1.35];...%SUZ
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP05_19\2025_FYP05_19_D2_R04.mat', [], 'SUZ', [-1.5];...%SUZ
    'D:\Users\Saian\Workspace\Data\+ProcessedData\2025\FYP05_19\2025_FYP05_19_D2_R03.mat', [2:8], 'SUZ', [-1.65];...%SUZ
};
...
for i = 1:size(runsLaps, 1)
    ...
    matFilePath = runsLaps{i,1};
     ...
     if ~isempty(runsLaps{i,2})
         ...
         for j = 1:numel(runsLaps{i,2})
             excludeLap_j = runsLaps{i,2};
             excludeLap_j = excludeLap_j(j);

            runData = runData(runData.lapNumber ~= excludeLap_j, :);
         end
...
for i = 1:size(data, 2)
    ...
    [dLookOverall, kappaSorted] = Utilities.fnLookAheadDistanceSigmoidCurvature(6, 40, 200, 0.02, kappaInterp);
    % [dLookOverall, kappaSorted] = Utilities.fnLookAheadDistanceSigmoidCurvature(8, 20, 200, 0.02, kappaInterp);
    ...
    for j = 1:size(data_i, 1)
        ...
        % Yaw Rate
        yawRate_j = prevSteering_j * yawRateGain;
        if abs(yawRate_j) < 0.01

            yawRate_j = 0;

        end

        yawRate(j,1) = yawRate_j;
    ...
    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, prevSteering1, yawRate, projectedCTE, lookAheadHE, data_i.steerAngle ];

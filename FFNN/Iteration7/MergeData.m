...
for i = 1:size(data, 2)
    ...
    dataArray_i = [data_i.lapNumber, data_i.CTE, ([0; diff(data_i.CTE)]), 1./data_i.rCurvature, ([0; diff(1./data_i.rCurvature)]), data_i.HeadingError, data_i.steerAngle ];
...
allData = array2table(dataArray, 'VariableNames',columnNames);

%% Use laps to create train, test, validation set
trainingData = allData(allData.lapNumber < 12, 2:end);
testData = allData(allData.lapNumber == 12, 2:end);
valData = allData(allData.lapNumber == 13, 2:end);
%% Export to CSV

writetable(trainingData, 'TrainingData.csv');
writetable(testData, 'TestData.csv');
writetable(valData, 'ValidationData.csv');
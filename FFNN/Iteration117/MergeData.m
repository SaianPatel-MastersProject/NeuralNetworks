...
for i = 1:size(data, 2)
    ...
    projectedCTE = zeros([size(data_i, 1), 5]);
    dX = diff(data_i.posX);
    dY = diff(data_i.posY);
    psi = atan2(dY, dX);
    psi = [psi; psi(end)];
    ...
    for j = 1:size(data_i, 1)
        ...
        % Project Errors
        for k = 1:5
            [projectedCTE_j, ~, ~, ~, ~] = PostProcessing.PE.fnCalculateProjectedPathError([xV, yV, psi(j)], AIW_Data, 8*k);
            projectedCTE(j,k) = projectedCTE_j  ;
        end
        % lookAheadHE(j,1) = lookAheadHE_j;
    ...
    dataArray_i = [data_i.CTE, lookAheadKappa_0, data_i.HeadingError, lookAheadKappa, prevSteering1, (prevSteering1 * yawRateGain), projectedCTE, data_i.steerAngle ];
...
%% View Input Space
plotInputSpace(trainingDataStand, [1:size(trainingDataStand, 2)]);

...
net = importNetworkFromONNX("SteeringModel_Iteration117.onnx", "InputDataFormats", "BC");
...
for i = 1:size(runStruct.data, 1)
    ...
    projectedCTE = zeros([1, 5]);
    for j = 1:5
        [projectedCTE_j, ~, ~, ~, ~] = PostProcessing.PE.fnCalculateProjectedPathError([xV, yV, thetaV], AIW_Data, j*8);
        projectedCTE(1, j) = projectedCTE_j;
    end
    ...
    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i, prevSteering1, prevSteering1*-1.5, projectedCTE];
    ...
    % Populate the array
    NN_Sim_Data(i,2:16) = input_i;
    NN_Sim_Data(i,17) = steeringOutput_i;
...
hold on
plot(NN_Sim_Data(:,1), NN_Sim_Data(:,17))
xlabel('Time (s)')
ylabel('steerAngle')
grid;
grid minor;

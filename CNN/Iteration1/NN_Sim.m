...
net = importNetworkFromONNX("SteeringModel_Iteration1.onnx", "InputDataFormats", "BTC");
...
for i = 1:size(runStruct.data, 1)
    ...
    yawRate = prevSteering1*-1.5;
    if abs(yawRate) < 0.01
        yawRate = 0;
    end

    input_i = [CTE, lookAhead_0, HeadingError, lookAhead_i, prevSteering1, yawRate, projectedCTE, lookAheadHE_i];
...
%% Sequencing
for i = 4:size(NN_Sim_Data,1)

    input_i = [NN_Sim_Data(i-3, 2:9); NN_Sim_Data(i-2, 2:9); NN_Sim_Data(i-1, 2:9); NN_Sim_Data(i, 2:9)];
    NN_Sim_Data(i, 10) = predict(net, input_i');
